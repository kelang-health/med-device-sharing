...
s = s.replace('v5.1.4 Secure Image Refresh', 'v5.1.5 Missing Image Recovery', 1)

needle = "const borrowImageErrorCache = new Map();"
helper = """function isLegacyImageUnavailableError(message){
    return String(message||'').includes('รูปหลักฐานเดิมนี้ไม่พร้อมใช้งานในคลังใหม่');
}
const borrowImageErrorCache = new Map();"""
if 'function isLegacyImageUnavailableError' not in s:
    if needle not in s:
        raise SystemExit('borrowImageErrorCache anchor not found')
    s = s.replace(needle, helper, 1)

old_hydrate = """        const err=borrowImageErrorCache.get(id)||'ไม่สามารถโหลดรูปหลักฐาน';
...
"""
new_hydrate = """        const err=borrowImageErrorCache.get(id)||'ไม่สามารถโหลดรูปหลักฐาน';
...
if old_hydrate not in s:
    raise SystemExit('hydrate error block not found')
s = s.replace(old_hydrate, new_hydrate, 1)

start = s.find('async function viewBorrowImages(entryId) {')
end = s.find('function closeImageGallery()', start)
if start < 0 or end < 0:
    raise SystemExit('viewBorrowImages block not found')
replacement = r'''async function viewBorrowImages(entryId) {
...
i = Path('index.html')
h = i.read_text(encoding='utf-8').replace('script.js?v=5.1.4', 'script.js?v=5.1.5')
i.write_text(h, encoding='utf-8')
print('patched v5.1.5')
